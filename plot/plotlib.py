...
from filter.filterlib import filter_eeg
...
def plot_time(eeg_file,
    ...
    if bandstop or bandpass:
        ...
        filtered_data = filter_eeg(data, fs, bandstop=bandstop, bandpass=bandpass, order=order)
    ...
    if xticklabels == 'seconds':
        xlabel = 'Time [s]'
        if sec_range:
            x_axis = np.arange(0, len(data_plotted) / float(fs), 1 / float(fs)) + sec_range[0]
        else:
    ...
    if threshold:
        ax.text(x_axis[0] - 0.12, 45, r'50', style='oblique', fontsize=30, color='red')
        # draw threshold line
        plt.plot((x_axis[0], x_axis[-1]), (threshold, threshold), 'r-', linewidth=linewidth, label='threshold')
...
def simulate_plot(data):
    ...
    ax.set_ylim(min(data_read) + 0.1 * min(data_read), max(data_read) + 0.1 * max(data_read))
    ax.hold(True)
...
class OnlinePlot(object):
    def __init__(self, samples_per_frame=4, channels=8):
        self.samples_per_frame = samples_per_frame
        self.count_frames = 0
        self.fig, self.ax = plt.subplots(channels, 1)

        # x axis limit is set to the 2 seconds of the OpenBCI signal
        for i in range(channels):
            self.ax[i].set_xlim(0, 250 * 2)
            self.ax[i].set_ylim(-10000, 10000)
            #     self.ax[i].set_autoscaley_on(True)
            self.ax[i].set_xlabel("Time(s)")

        self.data_plotted = [[0] * 2 * 250 for _ in range(channels)]
        ...
        self.signal = [0] * channels
        for i in range(channels):
            self.signal[i] = self.ax[i].plot(range(len(self.data_plotted[0])), self.data_plotted[i])[0]
        self.tic = time.time()
    ...
    def frame_plot(self, sample):
        ...
        for i in range(len(sample)):
            del (self.data_plotted[i][0])
            self.data_plotted[i].append(sample[i])

        if self.count_frames % self.samples_per_frame == 0:
            for i in range(len(sample)):
                self.signal[i].set_data(range(len(self.data_plotted[i])), self.data_plotted[i])

            # # restore background
            # 恢复background中的信息（也就擦除了所有动态元素）
            ...
            # 绘制更新后的动态对象
            for i in range(len(sample)):
                self.ax[i].draw_artist(self.signal[i])
